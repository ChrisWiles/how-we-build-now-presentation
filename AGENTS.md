...
- Make deep planning a visible conversation on Human Intent: AI asks clarifying questions first, people and AI discuss options and tradeoffs, then draft, critique, and revise through repeated feedback before agreeing on scope and evidence. Orchestration turns that plan into bounded tasks and sends new gaps back through planning.
...
- Review output is a skimmable decision brief: lead with decisions needing human judgment, then assess intent, architecture/reuse, risk/reversibility, performance, security/data, and verification as Checked, Concern, or Unknown with evidence and source links. Validate/deduplicate findings and keep local fix/recheck plus CI review. Humans challenge the big picture and inspect relevant code based on risk; a brief is not blanket approval. Keep changes focused, teach architectural reuse in domain skills, and calibrate delegation using review misses and regressions. Strengthen existing slides instead of expanding the deck with tool discussions.
